...
class COVID19Survey(FlaskForm):
    ...
    freeMassTesting = BooleanField("Nagsagawa ba ng free mass testing ang kumpanya sa hanay ng mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    dailyHealthMonitoring = BooleanField("Nagsasagawa ba ang kumpanya ng daily health monitoring tulad ng pag-check ng \
        body temperature at pagpapasagot ng daily health questionnaire sa mga manggagawa \
        bago pumasok sa loob ng lugar-paggawa*?", default="Oo", false_values=(False, "Wala"), validators=[])
    distancing = BooleanField("Naipatutupad ba ang physical at social distancing sa iba’t ibang erya ng \
        lugar-paggawa – production area, canteen, pantry, office, locker area, exit and \
        entrance, at iba pa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    supplements = BooleanField("Nagbigay ba ng libreng mga bitamina o iba pang pagkain/suplay na makatutulong \
        sa pagpapalakas ng resistensya ng mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    freePPE = BooleanField("Nagbibigay ba ang kumpanya ng libreng personal protective equipment (PPE) sa \
        mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    adequateSoapAndWater = BooleanField("May sapat bang suplay ng tubig at sabon sa lugar-paggawa para sa madalas na \
        paghuhugas ng mga kamay ng manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    freeRubbingAlcohol = BooleanField("May ipanamimigay bang libreng alcohol o hand sanitizer ang kumpanya sa mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    regularDisinfection = BooleanField("Nagsasagawa ba ng regular na disinfection ang kumpanya sa iba’t ibang erya sa \
        loob ng lugar-paggawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    hoursPerDay = IntegerField("Ilang oras ang pasok ninyo sa trabaho ngayong panahon ng pandemya?*")
    overtime = BooleanField("May overtime bang ipinapatupad ang kumpanya?*", default="Oo", false_values=(False, "Wala"), validators=[])
    covidInformationCampaign = BooleanField("Nagsasagawa ba ng information and awareness campaign ang kumpanya hinggil sa \
        COVID-19 at kung paano magiging ligtas sa impeksyon ang mga manggagawa at mapigilan \
        ang pagkalat nito sa lugar-paggawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    freeSafeAccomodation = BooleanField("Nagbigay ba ng libre at ligtas na akomodasyon ang inyong kumpanya para sa mga \
        manggagawang malayo ang inuuwian o para malimita rin ang kanilang exposure sa coronavirus?*", default="Oo", false_values=(False, "Wala"), validators=[])
    freeSafeAccomodationDetails = StringField("Kung oo, ilarawan ang akomodasyong ibinigay para sa mga manggagawa.")
    freeMedicalCheckup = BooleanField("Naglunsad ba ng libreng medical check-up sa hanay ng mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    covidRiskAssessment = BooleanField("Naglunsad ba ng risk assessment sa iba’t ibang erya sa lugar-paggawa para i-check \
        ang bentilasyon, maayos na layout, espasyo at iba pa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    isolationRoomPresent = BooleanField("May isolation room ba sa lugar paggawa para sa mga manggagawang magpapakita ng \
        mga sintomas ng COVID-19 sa panahon ng trabaho?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    COVIDCaseConfirmed = BooleanField("Nagkaroon na ba ng confirmed COVID case sa lugar-paggawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    ...
    companyPaysForTreatment = BooleanField("Sagot ba ng kumpanya ang pagpapagamot sa mga confirmed COVID case?*", default="Oo", false_values=(False, "Wala"), validators=[])
    contactTracing = BooleanField("Nagsagawa ba ng contact tracing ang kumpanya nang magkaroon ng probable o confirmed \
        COVID case sa lugar-paggawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    quarantineProcessForContacted = BooleanField("May mga quarantine procedure ba para sa mga contact?*", default="Oo", false_values=(False, "Wala"), validators=[])
    sickSalary = BooleanField("Kapag na-quarantine ang manggagawa, tuloy ba ang kanyang sahod?*", default="Oo", false_values=(False, "Wala"), validators=[])
    sickHealthMonitoring = BooleanField("Kapag na-quarantine ba ang manggawa, tuluy-tuloy ba ang pagmonitor ng kumpanya \
        sa kanyang kalusugan?*", default="Oo", false_values=(False, "Wala"), validators=[])
    healthInsurance = BooleanField("May health insurance bang ibinigay ang kumpanya sa mga manggagawa na maaaring \
        magamit para sa pagpapagamot kung sakaling magkaroon sila ng COVID-19?*", default="Oo", false_values=(False, "Wala"), validators=[])
    hazardPay = BooleanField("May hazard pay bang natatanggap ang mga manggagawa?*", default="Oo", false_values=(False, "Wala"), validators=[])
    admissionOfVulnerable = BooleanField("Pinapapasok ba ang bulnerableng seksyon ng mga manggagawa (may mga comorbidity \
        o pre-existing illness, high risk pregnancy o 60 years old and above)?*", default="Oo", false_values=(False, "Wala"), validators=[])
    OSHCommittee = BooleanField("May nakatayo bang occupational safety and health (OSH) committee?*", default="Oo", false_values=(False, "Wala"), validators=[])
    DOLEInspection = BooleanField("Nagsagawa ba ng inspeksyon ang Department of Labor and Employment para subaybayan \
        ang pagpapatupad ng kumpanya ng mga prevention and control measure para sa COVID-19?*", default="Oo", false_values=(False, "Wala"), validators=[])
    submit = SubmitField("Submit Form (Ipasa ang form)")
